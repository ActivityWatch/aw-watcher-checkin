...
def test_polling_loop_posts_window_block_only_when_enabled(monkeypatch, tmp_path, enabled):
    ...
    monkeypatch.setattr(cli, "prompt", lambda *_args, **_kwargs: "focused")
    ...
    if enabled:
        ...
        assert saved[0].data["message"] == "focused"
    else:
